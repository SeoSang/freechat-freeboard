...
export const login = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    res.status(200).cookie("token", token, CookieOptions).json(resUser)
  }
)
...
export const register = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    return res.status(200).cookie("token", token, CookieOptions).json(newUser)
  }
)
...
export const getUsers = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const exUsers = await users.findAll()
